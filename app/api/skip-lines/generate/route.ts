...
import {
  generateSkipLines,
  normalizeSkipLineRequest,
  type SkipLineNoContentReason,
} from "./pipeline";

function noContent(reason: SkipLineNoContentReason): NextResponse {
  return new NextResponse(null, {
    status: 204,
    headers: {
      "X-WAIV-Skip-Line-Reason": reason,
    },
  });
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const input = normalizeSkipLineRequest(rawBody);
  if (!input) {
    return NextResponse.json({ error: "Missing required fields." }, { status: 400 });
  }

  try {
    const result = await generateSkipLines(input);
