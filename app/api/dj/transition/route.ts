...
import {
  generateTransitionCommentary,
  normalizeTransitionRequest,
  type TransitionNoContentReason,
} from "./pipeline";

function noContent(reason: TransitionNoContentReason): NextResponse {
  return new NextResponse(null, {
    status: 204,
    headers: {
      "X-WAIV-Transition-Reason": reason,
    },
  });
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const input = normalizeTransitionRequest(rawBody);
  if (!input) {
    return NextResponse.json({ error: "Missing required fields." }, { status: 400 });
  }

  try {
    const result = await generateTransitionCommentary(input);
